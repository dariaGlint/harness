...
def _verify_published_branch_head(
    client: GitHubCommitClient,
    repository: str,
    branch_name: str,
    expected_sha: str,
) -> None:
    """Verify a newly written ref while tolerating brief GitHub read-after-write lag."""
    ...
    for delay in delays:
        ...
        if published_head == expected_sha:
            return
        if published_head is not None:
            break
    raise GitHubPublicationError(
        "branch_verification_mismatch",
        f"branch verification failed: expected {expected_sha}, got {published_head}",
    )
...
def _publish(
    ...
    *,
    ...
    _verify_published_branch_head(client, repository, branch_name, commit_sha)
